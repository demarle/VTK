...
void PrepArray(bool byName, bool drawCell, unsigned int arrayIndex, unsigned int arrayComponent,
...
*/
  ...
  for (int i = 0; i < 8; i++)
    ...
    for (unsigned int j = 0; j < fd->GetNumberOfArrays(); j++)
      {
      for (unsigned int k = 0; k < fd->GetArray(j)->GetNumberOfComponents(); k++)
        {
