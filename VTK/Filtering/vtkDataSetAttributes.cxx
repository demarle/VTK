    ...
    for(i=this->RequiredArrays.BeginIndex(); !this->RequiredArrays.End();
      ...
      if (shallowCopyArrays)
        ...
        if ( sze > 0 )
          ...
          {
          newAA->Allocate(aa->GetNumberOfTuples()*aa->GetNumberOfComponents());
          }
...
{
  int i;
  for(i=0; i<this->RequiredArrays.GetListSize();++i)
    {
    int j = this->RequiredArrays.GetIndex(i);
    this->CopyTuple(fromPd->Data[j], this->Data[this->TargetIndices[j]],
                    fromId, toId);
    }
}
